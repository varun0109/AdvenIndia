 ...
 (function ($, window, document, undefined) {
    ...
    $.extend(bstreeView.prototype, {
        ...
        build: function (parentElement, children, depth) {
            ...
            depth += 1;
            // Add each node and sub-children.
            $.each(children, function addNodes(id, node) {
                ...
                // Set node Text.
                 ...
                 if (parseInt(node.BV) > 0 ){
                    treeItem.append('<span ><img style="width:10%;height:10%;border-radius:50%" src="'+node.image+'" </span><span> </span><b class="treeid_active" style="color:green">'+node.fullname + '</b>' +'<div class="tree-bv"> Purchase Point  '+node.purchase_points+ '   <span class="tree-plan-act" > Active </span>'+ '   <span class="tree-plan-lvl" > level '+ node.level+' </span></div>' );
                
                 }
                 else{
                    treeItem.append('<span><img style="width:10%;height:10%;border-radius:50%" src="'+node.image+'" </span><span> </span><b class="treeid_active">'+node.fullname + '</b>' +'<div class="tree-bv"> Purchase Point  '+node.purchase_points+ '   <span class="tree-plan-inc" > Inactive </span>' + '   <span class="tree-plan-lvl" > level '+ node.level+' </span></div>' );
